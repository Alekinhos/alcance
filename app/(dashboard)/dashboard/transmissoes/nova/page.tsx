...
import { criarTransmissao } from '@/app/actions/transmissoes'
import { Campo, Textarea } from '@/components/ui/campo'
import { Botao } from '@/components/ui/botao'
import { ArrowLeft } from 'lucide-react'
import Link from 'next/link'

export default function PaginaNovaTransmissao() {
  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    const resultado = await criarTransmissao(dados)
  ...
  return (
    <div className="p-6">
      <div className="mb-6">
        ...
        <h1 className="text-2xl font-bold text-porta">Nova Transmissão</h1>
      </div>

      <div className="max-w-2xl rounded-lg border border-pao bg-white p-6">
        <form onSubmit={handleSubmit} className="space-y-4">
          <Campo label="Título" name="titulo" required placeholder="Ex: Culto de Domingo — Manhã" />
          <Textarea label="Descrição" name="descricao" placeholder="Tema, pregador, etc. (opcional)" />
          <Campo
          ...
          <div className="flex justify-end gap-3 pt-2">
            ...
            <Botao type="submit" carregando={carregando}>Salvar</Botao>
          </div>
        </form>
      </div>
    </div>
